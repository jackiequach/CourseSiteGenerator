package djf.settings;

/**
 * This enum provides properties that are to be loaded via
 * XML files to be used for setting up the application.
 * 
 * @author Richard McKenna
 * @author Jackie Quach
 * @version 1.0
 */
public enum AppPropertyType {
    // LOADED FROM app_properties.xml

    // INITIAL WINDOW TITLE AND LOGO
    APP_TITLE,
    APP_LOGO,
    
    // FOR FINDING THE CSS FILE
    APP_CSS,
    APP_PATH_CSS,

    // FILE TOOLBAR ICONS
    NEW_ICON,
    LOAD_ICON,
    SAVE_ICON,
    SAVE_AS_ICON,
    EXIT_ICON,
    EXPORT_ICON,
        
    // FILE TOOLBAR TOOLTIPS FOR BUTTONS
    NEW_TOOLTIP,
    LOAD_TOOLTIP,
    SAVE_TOOLTIP,
    SAVE_AS_TOOLTIP,
    EXPORT_TOOLTIP,
    EXIT_TOOLTIP,
    
    // EDIT TOOLBAR ICONS
    UNDO_ICON,
    REDO_ICON,
    ABOUT_ICON,
    
    // EDIT TOOLBAR TOOLTIPS FOR BUTTONS
    UNDO_TOOLTIP,
    REDO_TOOLTIP,
    ABOUT_TOOLTIP,
	
    // ERROR MESSAGES AND TITLES FOR DIALOG BOXES
    NEW_ERROR_MESSAGE,
    NEW_ERROR_TITLE,
    LOAD_ERROR_MESSAGE,
    LOAD_ERROR_TITLE,
    SAVE_ERROR_MESSAGE,
    SAVE_ERROR_TITLE,
    EXPORT_ERROR_TITLE,
    EXPORT_ERROR_MESSAGE,
    PROPERTIES_LOAD_ERROR_MESSAGE,
    PROPERTIES_LOAD_ERROR_TITLE,
    INVALID_START_TIME_TITLE,
    INVALID_START_TIME_MESSAGE,
    INVALID_END_TIME_TITLE,
    INVALID_END_TIME_MESSAGE,
    INVALID_START_MONDAY_TITLE,
    INVALID_START_MONDAY_MESSAGE,
    INVALID_END_FRIDAY_TITLE,
    INVALID_END_FRIDAY_MESSAGE,
    INVALID_START_MONDAY_MONDAY_TITLE,
    INVALID_START_MONDAY_MONDAY_MESSAGE,
    INVALID_END_FRIDAY_FRIDAY_TITLE,
    INVALID_END_FRIDAY_FRIDAY_MESSAGE,
	
    // AND VERIFICATION MESSAGES AND TITLES
    NEW_COMPLETED_MESSAGE,
    NEW_COMPLETED_TITLE,
    LOAD_COMPLETED_MESSAGE,
    LOAD_COMPLETED_TITLE,
    SAVE_COMPLETED_MESSAGE,
    SAVE_COMPLETED_TITLE,
    EXPORT_COMPLETED_TITLE,
    EXPORT_COMPLETED_MESSAGE,
    SAVE_UNSAVED_WORK_TITLE,
    SAVE_UNSAVED_WORK_MESSAGE,
    ABOUT_TITLE,
    ABOUT_MESSAGE,
    SAVE_WORK_TITLE,
    LOAD_WORK_TITLE,
    DELETE_TIME_TITLE,
    DELETE_TIME_MESSAGE,
    CALENDAR_BOUND_TITLE,
    CALENDAR_BOUND_MESSAGE,
    BANNER_TITLE,
    LEFT_FOOTER_TITLE,
    RIGHT_FOOTER_TITLE,
    EXPORT_WORK_TITLE,
    EXPORT_DIR_TITLE,
    COURSEDETAILS_TAB_TEXT,
    TADATA_TAB_TEXT,
    RECITATIONDATA_TAB_TEXT,
    SCHEDULEDATA_TAB_TEXT,
    PROJECTDATA_TAB_TEXT,

    // DATA FILE EXTENSIONS AND DESSCRIPTIONS
    WORK_FILE_EXT,
    WORK_FILE_EXT_DESC
}
